# 보스 아트 생성 프롬프트 (AI 이미지 에셋) — ⏸ 보류(2026-07-06)

> **결정(2026-07-06): 이 AI 에셋 방향은 보류. 보스는 게임 스타일의 절차 생성으로 간다(방향 A).**
> 이유: ① AI esports 마스코트는 디테일이 많아 진행 방향으로 회전시키면 뒤집혀 보인다(중력 있는 측면 포즈).
> ② 우리 게임 생물(굵은 단일 윤곽선·플랫 2~3톤·큰 단순 눈)과 스타일이 안 맞아 보스만 붕 떠 보인다.
> → `render/worldView.ts` 의 `drawBossCreature`(절차)를 게임 스타일로 다듬어 위압적이면서 회전 안전하게 만든다.
> **나중에 방향이 바뀔 수 있어** 이 프롬프트 문서는 지우지 않고 참고용으로 남긴다(단순·플랫·탑다운으로 다시
> 뽑거나, flip-only 통합을 재검토할 여지).

아래는 (보류된) AI 생성 레시피다. 프롬프트를 그대로 복사해 GPT 에 붙이면 마젠타 배경·흰 테두리 없는 측면
마스코트가 나온다. 다시 쓰게 되면 스타일을 우리 생물에 맞춰 훨씬 단순·플랫·탑다운으로 바꿔야 한다.
