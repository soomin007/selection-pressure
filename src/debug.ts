...
//   ?head=<0~1>  헤딩(진행방향) 저역통과 세기. 작을수록 회전이 더 안정(좌우 진동 제거)·전환 느림. 1=끔.
...
const tuned =
  params.has("dz") || params.has("rotk") || params.has("smooth") || params.has("head");
...
export function debugLabel(): string {
  ...
  if (params.has("head")) on.push(`head=${TUNE.headingSmooth}`);
  return on.join(" · ");
}
