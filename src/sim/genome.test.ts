...
describe("게놈 v2 마이그레이션", () => {
  it("기본 게놈은 v2 + 모든 형질 0.5(수영 포함)", () => {
    ...
    for (const k of TRAIT_KEYS) expect(g.traits[k]).toBe(0.5);
    expect(g.traits.swimming).toBe(0.5);
  });
  ...
  it("v1 게놈을 받으면 swimming 0.5 를 채워 v2 로 올린다", () => {
    ...
    expect(g.genomeVersion).toBe(2);
    expect(g.traits.speed).toBe(0.7);
    expect(g.traits.diet).toBe(0.8);
    expect(g.traits.swimming).toBe(0.5); // v1 엔 없던 형질을 기본값으로 채움
  });

  it("직렬화 왕복이 보존된다", () => {
    const g = defaultGenome();
    g.traits.swimming = 0.9;
    const round = deserializeGenome(serializeGenome(g));
    expect(round.traits.swimming).toBe(0.9);
    expect(round.genomeVersion).toBe(2);
  });
