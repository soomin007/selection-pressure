 ...
 *
 * **아래 표는 계산이 아니라 실측이다** (`npm run probe -- tiers` · 실제 72장 풀 · 희귀도 가중 ·
 * 소프트 디듑 · 3000런). 카드가 주는 도장은 「한 범주 최고」 기준이다.
 *
 * | 어떻게 골랐나 | 12장 | 17장 | 22장 |
 * |---|---|---|---|
 * | 한 범주만 판다 | 16.0 (III) | **20.0 (IV 69%)** | 22.4 (IV 95%) |
 * | 두 범주를 판다 | 15.6 / 10.8 (III+II) | **19.5 / 14.6 (듀오 74%)** | 22.0 / 18.7 (IV+III) |
 * | 큰 숫자만 고른다 | 11.8 / 9.1 (II+II) | 15.1 / 11.8 (III+II) | 17.9 / 14.8 (III+III) |
 * | 아무거나 고른다 | 10.7 / 8.2 (II · **3단 0.12개**) | 13.5 / 10.4 (II~III) | 16.1 / 12.9 (III+II) |
 *
 * 마지막 줄이 이 재설계의 목표다. 예전엔 정점 넷이 런 끝에 **반드시** 다 찍혔는데, 이제 손 놓은
 * 판은 **관문 자격을 하나도 못 얻는다**(3단이 평균 0.12개). 그리고 다섯 범주 전부 4단은 원리적으로
 * 불가능하다 — 그러려면 100 도장이 필요한데 가장 후한 판의 공급이 약 30이다.
 * **[사용자]** 최고 티어는 대멸종을 세 번 넘긴 뒤(시대 4)에 열린다 = 누적 20.
 *
 * ⚠ **이 표는 프리셋 시작 도장 6(주 4 + 부 2)을 전제로 한다.** 프리셋을 만지면 사다리 전체가
 *   움직이므로 반드시 `npm run probe -- tiers` 를 다시 돌려라.
 */
...
//   낮추는 것이 아니다.
//   ⚠ 3단이 100 을 넘으면 안 된다( 가 규칙 면제를 켠다). 4단만 100 을 넘긴다.
