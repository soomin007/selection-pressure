...
//   · 고블린은 `world.entities` 에 없다. 야생도 내 종도 그를 감지·추적·공격하지 않는다.
//   · 무작위는 전용 `world.goblinRng` 만 쓴다(geneRng 와 같은 결) · 메인 rng 를 쓰면 야생 스폰·진화가
//     통째로 이동한다.
//   · 세계에 남기는 흔적은 계수(`roundCounts.marked`)와 연출 사건뿐이다. **시험이 안 걸린 세계
//     (goblinQuota 0)에서는 이 파일의 코드가 한 줄도 안 돈다** — 기존 세계는 1비트도 안 바뀐다.
//
...
export const GOBLIN = {
   ...
   * ⚠ **이 값은 폰 실기로만 정해진다**(0.88 은 출발값 · 내 판단) — 「몰아야 잡힌다」의 손맛이 전부다.
   */
  speedRatio: 0.88,
