...
export interface Entity {
  ...
  wanderAngle: number;
  // 누적 독 피해 풀(런타임, 직렬화 안 함). venom 공격에 물리면 쌓이고, 매 틱 에너지를 깎으며 소진된다.
  poison: number;
  // 이 개체에 마지막으로 독을 건 포식자(런타임, 직렬화 안 함). 독으로 죽으면 그 포식자가 먹이 에너지를
  // 얻는다 — 독 종이 "천천히 죽이는" 사냥에서도 이득을 보게(즉사와 동등). 죽었으면 무시.
  poisonSource: Entity | null;
