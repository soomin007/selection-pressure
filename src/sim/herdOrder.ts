...
//   · 행군 중에도 **발밑의 먹이는 스치며 먹는다**(걸음은 안 바꾼다 · behavior 섭취 블록의 스침 채집).
...
//   도망 > 사냥감 추적 > **방울** > 지시 > 배회. **먹이는 이동을 못 가져간다**(스침 채집으로만).
//   · 「가는 길의 먹이」 예외(지시 쪽·코앞이면 그것부터)는 2026-08-12 에 걷어냈다 · 그 예외 하나가
//     순종률을 30.8%p 새게 했다(2026-08-08 실측 · 순종 처방 셋의 ② · backlog 5번).
...
//     (world.orderPending)다 · 도망 중인 개체도 분모에 든다(그래서 N < M 이 정상). 다만 하나는
//     여전히 지시보다 위다:
...
//
// ⚠ 이 경계는 두 번의 실측이 그었다. 이 파일은 숫자가 아니라 **계약**이다 · 코드와 이 주석이
// 갈리면 다음 세션이 주석을 근거로 또 잘못 판단한다. 조건문을 고치면 위 문장부터 고칠 것.
//   · 2026-08-04: "먹이·사냥 목표가 있으면 지시를 통째로 무시"는 순종률 7.5% 를 만들었다 · 먹이를
//     쫓는 것은 예외가 아니라 **기본 상태**였다(개체틱의 72.1%) → 「가는 길 먹이」만 예외로 남겼다.
//   · 2026-08-08: 그 예외도 순종률을 30.8%p 새게 했다(순종 17.2%) → 2026-08-12 에 예외를 걷어내고
//     **스침 채집**(행군 중 발밑 먹이를 걸음 안 바꾸고 먹는다)으로 굶주림을 보전했다. 목소리 520 ·
//     예외 제거 · 스침 채집 셋이 **한 묶음 처방**이다(하나만 넣으면 안 된다 · backlog 5번).
