...
async function boot(): Promise<void> {
  ...
  function issueOrder(wx: number, wy: number, kind: OrderKind = "move"): OrderResult {
    ...
    if (game.leadVacuumSeconds > 0) {
      ...
      // ⚠ **문구는 자기 완결적이어야 한다**(2026-08-09 · [사용자] 두 번 지적).
      //   옛 문구는 "앞장서던 것이 쓰러졌습니다. 무리가 잠시 흩어집니다" 였는데 둘 다 문제였다:
      //   ① **앞장서는 개체가 있다는 것을 화면이 한 번도 안 가르쳐 준다.** 무리 지시로 갈아탈 때
      //      알파 안내 넷을 전부 지웠으면서(아래 1356행 주석) 이 죽음 알림만 남겨 뒀다 —
      //      그래서 처음 보는 사람에게는 "뭔 앞서가던 애"라는 뜬금없는 말이 된다.
      //   ② **"흩어집니다"는 사실이 아니다.** 무리는 그대로 있고, 잠시 **명령이 안 닿을** 뿐이다.
      //   그래서 무엇이 일어났는지와 **어떻게 되돌리는지**를 한 줄에 담는다. 되돌리는 법은 실제로
      //   맞다: 내 개체를 탭하면 `passBaton` 이 공백을 0 으로 밀어 그 자리에서 명령이 다시 통한다.
      highlights.flash("이끌던 개체가 쓰러졌습니다. 내 개체를 탭하면 그 애가 앞장섭니다", 0xd07050);
      return { ok: false, told: true };
    }
