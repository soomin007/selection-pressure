...
async function boot(): Promise<void> {
  ...
  // ── 폰에서 화면 아래가 검게 남던 것 (2026-08-10 · **[사용자]** 제보) ────────────────────────
  //
  // `resizeTo: window` 는 `window.resize` 만 듣는다. 그런데 **모바일 브라우저의 주소창이 접히는
  // 순간에는 그 이벤트가 안 오거나 늦게 온다**(iOS Safari·안드로이드 Chrome 공통). 그러면
  // 보이는 뷰포트만 커지고 캔버스는 옛 높이 그대로라 **아래에 배경색 띠가 남는다.**
  // 사용자가 "갑자기 어쩌다 보니 사라졌다"고 한 것도 이것으로 설명된다 — 회전이나 키보드처럼
  // 진짜 `resize` 를 일으키는 일이 한 번 생기면 그 자리에서 고쳐진다.
  //
  // `visualViewport` 는 **실제로 보이는 영역**을 알려 주고 주소창이 접힐 때도 이벤트를 준다.
  // 그것을 함께 들어 캔버스를 다시 맞춘다. rAF 로 미루는 이유: 접히는 도중에 여러 번 오는데,
  // 그때마다 렌더 타깃을 다시 만들면 폰에서 프레임이 튄다.
  //
  // ⚠ **재현은 못 했다.** 증상(아래만 검다 · 어쩌다 저절로 낫는다)이 이 함정과 정확히 맞고 고치는
  //   비용이 작아서 넣었지, 이것이 그 버그였다고 **확인한 것은 아니다.** 폰에서 다시 나오면
  //   여기가 아니라 다른 데를 봐야 한다(다음 후보: `chooseLayout` 의 초기 1회 계산 · CSS `height:100%`).
