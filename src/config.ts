...
// ⚠ **측정 도구의 단일 근원**: 실제 플레이 세계 치수는 여기서만 나온다. main.ts(게임 본편) ·
// src/sim/boss.test.ts(격퇴 테스트) · scripts/balance-probe.mjs(밸런스 프로브)가 전부 이 값을
// 읽는다. 복사본을 만들면 맵 크기를 바꿀 때 측정 도구가 존재하지 않는 세계를 재게 된다
