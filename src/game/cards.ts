 ...
 *
 * 2026-08-13 결정 회의(안건 2 · A안 · [사용자] 위임) 재설계. 실측이 잡은 결함은 **전설(열쇠)의
 * 조기 홍수**였다: 열쇠 일곱은 게이트가 없어 첫 드래프트부터 열리는데, 옛 가중치(10)로는 시대 1
 * 후보의 2.0%, 시대 3 에는 3.7% 로 **드묾(3.2%)보다 자주 떠** 서열이 뒤집혔다(growth 16시드 ·
 * 후보 1,682장 실측). 전설 4 는 첫 판 풀(흔함 10 + 열쇠 7)에서 후보당 약 2.7% = 목표 분포의
 * 「전설 2%」에 닿는 값이고, 열쇠 빌드의 문은 아래 보정(레벨이 오르면 ×9)이 도로 연다.
 *
 * ⚠ **등급 분포의 나머지 절반은 가중치가 못 고친다.** 귀함은 전부 2단이, 아주 귀함은 전부 3단
 *   게이트가 연다(등급이 게이트 깊이와 묶여 있다) — 방울을 안 쓰는 자(무구입 프로브)에서 0.0% 인
 *   것은 가중치가 아니라 **자의 한계**다. 목표 분포(흔함 55 · 드묾 25 · 귀함 12 · 아주 귀함 6 ·
 *   전설 2%)의 최종 검증은 구입 정책 자(backlog 2번)가 선 뒤에만 가능하다.
 */
 ...
 * 18 → 14 (2026-08-13 결정 회의 안건 2 · C안): 실측 최종 레벨이 12.1~20.5(프리셋 평균 · growth
 * 시드 8)라 18 은 대부분의 런이 보정 최대에 영영 못 닿는 값이었다. 14 면 **런 내내 계속 오르다
 * 끝머리에서 만배에 닿는다** — 「런 내내 오른다」는 사용자 방향은 그대로다.
 */
export const RARITY_BOOST_FULL_LEVEL = 14;

/**
 * 최대 보정에서 각 등급의 가중치가 기준값의 몇 배가 되는가. 흔함은 1배(그대로)이고 높은 등급만 커진다
 * → 흔함의 **몫**이 자연히 줄어든다. "무리가 자라면 더 큰 변화가 찾아온다"를 확률로 표현한 것.
 *
 * 전설 ×9 는 기준값을 4 로 내린 것과 짝이다(위 주석): 첫 판에서 귀하고(4), 레벨 14 에서 36 —
 * 옛 값(10 → 55)보다 늦게, 덜 흔하게 열리되 열쇠 빌드가 죽지는 않는 자리를 실측으로 재라.
 * ⚠ 흔함~귀함의 **한 장당 서열**(귀함 < 드묾 < 흔함)은 모든 레벨에서 지켜야 한다(cards.test 계약).
 *   보정 최대에서 92.8 < 97.5 < 100 으로 아슬하게 선다 — 배수를 키울 때 이 사다리부터 확인하라.
 */
