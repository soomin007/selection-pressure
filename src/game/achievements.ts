...
export const ACHIEVEMENTS: readonly Achievement[] = [
  ...
  },
  {
    ...
    // 통째로 면제받는 자리다.
    //
    // ⚠ **보상 카드 id 를 v9 에서 또 갈아야 했다.** 옛 값 `lp_hide2`(「바위 같은 등」 · 가죽 +3)는
    //   도장 카드라 카드 재설계와 함께 사라졌다. 지금은 같은 결의 특성 카드 「바위 살갗」이다
    //   (늘 · 받는 피해 ×0.79 · 가죽 계열에서 가장 큰 장). **이 자리는 카드 id 가 바뀔 때마다 조용히
    //   죽는다** — 2026-08-07(v8 전환)과 2026-08-10(v9 전환)에 두 번 겪었고, 죽어도 게임이 멀쩡히
    //   돌아가서 아무도 눈치채지 못한다. `achievements.test.ts` 가 이제 「보상 카드 id 가 실제 풀에
    //   있다」를 못박는다(meta.ts 의 해금표가 같은 이유로 같은 검사를 갖고 있다).
    desc: "가죽을 최고 단계까지 키운 종으로 승리한다.",
    reward: { kind: "card", cardId: "pk_defense_rock" },
