 ...
 * - 값형질(속도·시야·공격·번식)·대사 = **숫자**(상한 100 이라 "68 = 68%"로 그대로 직관적). 드래프트에서
 *   카드 효과(+9)를 수치로 비교해 뽑으므로 숫자가 맞다(사용자 방향).
 * - 능력형(수영·날개·초음파·독·원거리·무리·은신) = 없음/보통/강함(v7: 무리가 여기로 내려왔다). 이 형질의
 *   카드 변화도 "강화/약화"(낱말)로 말한다 — 값 델타(+8)를 붙이면 "강함 +8"처럼 낱말과 숫자가 뒤섞인다.
 * - 식성 = 초식/잡식/육식(스펙트럼).
 */
