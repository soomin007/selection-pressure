...
//  ② **못 여는 칸이 보인다.** 잠긴 칸을 감추지 않고 회색으로 그린다. 그래야 "다리 1단이 되면
//     「피해라」가 열린다"를 대백과가 아니라 **손에서** 알게 된다(CLAUDE.md 전달 규칙).
//     ⚠ 2026-08-09 현재 실제로 도는 칸은 **「가라」와 「피해라」 둘뿐**이다. 나머지 여섯은 sim 에
//       분기가 없어 「가라」와 완전히 같게 굴면서 설명·쿨타임·기력만 달고 있었다(= 벌칙만 붙은
//       「가라」). 지금은 `OrderSpec.ready: false` 로 잠겨 있고 힌트가 "아직 준비 중입니다"라 말한다.
//       회색 칸이 「티어로 열린다」와 「아직 안 만들었다」 둘 다를 같은 자리에서 말하는 셈이라,
//       구현되는 대로 hint 를 티어 문구로 되돌리면 성장 신호가 그대로 살아난다.
//
