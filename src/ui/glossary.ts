...
const DUOS_ENTRY: Entry = {
  ...
  svg: SVG.herding,
  desc:
    // ⚠ 2026-08-10 에 정반대가 됐다. 예전엔 「저절로 켜지고 카드가 따로 없다」였는데, 듀오가
    //   드래프트 카드가 되면서 **고르지 않으면 안 켜진다.** 옛 문구를 그대로 뒀다면 이 항목이
    //   화면에서 가장 크게 거짓말하는 자리가 됐을 것이다.
    `두 범주가 함께 ${TIER_ROMAN[DUO_TIER]} 이상이 되면 **드래프트에 나타나는** 합체 형질 카드입니다. ` +
    "두 기둥을 깊게 판 종만 만날 수 있고, 나타나도 그 카드를 골라야 켜집니다.",
...
const SECTIONS: readonly Section[] = [
  ...
  },
  {
    title: "위협 도감",
    intro:
      "보스와 대멸종은 라운드 시련이 아니라 관문입니다. 끝까지 살아남으면 넘어갑니다.\n" +
    ...
    entries: [
      {
        term: "한 판의 흐름",
        svg: SVG.flow,
        desc: "시작 종을 고르고, 화면을 탭해 우두머리를 이끕니다. 탭한 곳이 사냥할 수 있는 상대면 쫓아가 물고, 그 밖의 곳이면 그리로 갑니다. 나머지 무리는 스스로 살면서 우두머리를 따라옵니다. 맨 처음 하는 판은 무리를 먹여 키우는 것만 하면 되고, 시대를 넘을 때마다 시련·지형·예전에 기르던 종이 한 가지씩 늘어납니다. 마지막 대멸종까지 살아남으면 승리입니다.",
      },
      {
        term: "라운드 시련과 불씨",
        svg: SVG.trophy,
        desc: "채집 라운드마다 시련이 하나 닥칩니다. 사냥 몇 번, 먹이 몇 번, 새끼 몇 마리, 무리 몇 마리처럼 그 라운드 안에 채워야 하는 목표입니다. 목표는 라운드가 시작되기 전 카드 고르는 화면에 미리 나오니, 그걸 보고 카드를 고르세요. 맨 처음 하는 판의 첫 시대에만 시련이 없습니다(그때는 무리를 먹여 키우는 것만 배웁니다).",
        note: "불씨는 이 혈통에 남은 기회입니다. 다섯 개로 시작해 시련에 질 때마다 하나씩 꺼지고, 모두 꺼지면 판이 끝납니다. 시련이 없는 동안에는 불씨도 줄지 않습니다. 보스를 물리치거나 다음 시대로 넘어가면 하나 되살아납니다.",
      },
      {
        term: "카드 고르기",
        svg: SVG.card,
      ...
      },
      {
        term: "이기는 법",
        svg: SVG.trophy,
        desc: "마지막 대멸종까지 내 종이 살아남으면 승리합니다. 지는 길은 둘입니다. 무리가 한 마리도 남지 않거나, 시련에 거듭 져 불씨가 다 꺼지거나. 한 형질만 극단으로 올리기보다, 다가오는 위협에 맞춰 균형을 잡는 것이 안전합니다.",
      },
    ],
  },
];
