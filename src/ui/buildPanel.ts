...
//
// ⚠ **v9 의 빈칸: 특성이 여기 없다.** 카드가 주는 것이 도장에서 조건부 특성(`sim/perks.ts`)으로
//   바뀌었는데, 이 패널은 여전히 「고른 카드」를 **이름만** 나열한다 ▸ 「밤눈」이 언제 얼마나 세지는지
//   판 안에서 볼 자리가 없고, 지금 그 특성이 켜져 있는지는 더더욱 안 보인다(`perks.activePerks` 가
